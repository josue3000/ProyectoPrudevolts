                ...
                <div class="col-lg-4 col-sm-4 col-md-4 col-xs-12" hidden>
                    <div class="form-group">
                        ...
                        <select name="estado_pago" id="" class="form-control" style="background-color: LightSkyBlue">
                            ...
                            <option value="Cancelado">Cancelado</option>
                            <option value="Deuda">Deuda</option>
                            <option value="Abortado" selected>Abortado</option> --}}
                        @endif
                ...
                <table id="detalles_servicio" class="table table-striped table-bordered table-condensed table-hoover">
                    ...
                    <tbody>
                        ...
                        <tr class="selected" id="{{'fila_s'.$loop->iteration}}">
                            ...
                            <td hidden><input type="number" name="precio_venta_servicio[]" value="{{$det_s->precio_venta_servicio}}"></td>
                            <td hidden><input type="number" name="descuento_servicio[]" value="{{$det_s->descuento_servicio}}"></td>
                            <td hidden><input type="hidden" id="{{'subtotal_s'.$loop->iteration}}" value="{{$det_s->precio_venta_servicio-$det_s->descuento_servicio}}">  {{$det_s->precio_venta_servicio-$det_s->descuento_servicio}}</td>
                        </tr>
                        @if($loop->first)
                ...
                {{-- <div class="panel panel-primary"> --}}
                    <div class="panel-body">
                        ...
                        <div class="col-lg-4 col-sm-4 col-md-4 col-xs-12">
                            <div class="form-group">
                                ...
                                <select name="aux_id_material" class="form-control selectpicker" id="aux_id_material" data-live-search="true">
                                    ...
                                    <option value="{{$material->id_material}}_{{$material->stock}}_{{$material->precio_promedio}}">{{$material->material}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <table id="detalles" class="table table-striped table-bordered table-condensed table-hoover">
                                ...
                                <tbody>
                                    ...
                                    <tr class="selected" id="{{'fila'.$loop->iteration}}">
                                        ...
                                        <td hidden><input type="number" name="precio_venta_material[]" value="{{$det->precio_venta_material}}"></td>
                                        <td hidden><input type="number" name="descuento[]" value="{{$det->descuento}}"></td>
                                        <td hidden><input type="hidden" name="subtotal[]" id="{{'subtotal'.$loop->iteration}}" value="{{$det->cantidad_material*$det->precio_venta_material-$det->descuento}}"> {{$det->cantidad_material*$det->precio_venta_material-$det->descuento}}</td>
                                    </tr>
                                    @if($loop->first)
                                    <p class="hidden"> Our first element of the array</p>
                                    @endif
                        ...
                        {{-- <div class="col-lg-9 col-sm-9 col-md-9 col-xs-12"> --}}
                            <label  class="col-lg-2 col-sm-2 col-md-2 col-xs-12" hidden for="importe_total">IMPORTE TOTAL: Bs/.</label>
                            ...
                            <label  class="col-lg-2 col-sm-2 col-md-2 col-xs-12" hidden for="importe_total">IMPUESTOS: Bs/.</label>
                            <div class="form-group col-lg-4 col-sm-4 col-md-4 col-xs-12" hidden>
   ...
   //subtotal = $('#subtotal');
    ...
    function agregar()
    {
        ...
        if (id_material!="" && material!="" && cantidad_material!="" && cantidad_material>0 && descuento!="" && precio_venta_material!="")
        {
            ...
            var fila='<tr class="selected" id="fila'+cont+'"><td><button type="button" class="btn btn-warning" onclick="eliminar('+cont+');">X</button></td><td><input type="hidden" name="id_material[]" value="'+id_material+'">'+material+'</td><td><input type="number"  name="cantidad_material[]" value="'+cantidad_material+'"></td><td hidden><input type="number"  name="precio_venta_material[]" value="'+precio_venta_material+'"></td><td hidden><input type="number"  name="descuento[]" value="'+descuento+'"></td><td hidden>'+subtotal[cont]+'</td></tr>';
            cont++;
            limpiar();
            $("#aux_total_repuestos_2").html("Bs/."+total);
 ...
 $(document).ready(function()
        ...
        function agregar_servicio()
        {
            ...
            if (id_servicio!="" && servicio!=""  && descuento_servicio!="" && precio_venta_servicio!="")
            {
                ...
                var fila_s='<tr class="selected" id="fila_s'+conta+'"><td><button type="button" class="btn btn-warning" onclick="eliminar_servicio('+conta+');">X</button></td><td><input type="hidden"  name="id_servicio[]" value="'+id_servicio+'">'+servicio+'</td><td hidden><input type="number"  name="precio_venta_servicio[]" value="'+precio_venta_servicio+'"></td><td hidden ><input type="number"  name="descuento_servicio[]" value="'+descuento_servicio+'"></td><td hidden>'+subtotal_s[conta]+'</td></tr>';
                conta++;
                limpiar_servicio();
                $("#aux_total_servicio_2").html("Bs/."+total_s);
